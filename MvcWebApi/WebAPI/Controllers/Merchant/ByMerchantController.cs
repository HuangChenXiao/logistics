...
namespace WebAPI.Controllers.Merchant
{
    public class ByMerchantController : ApiController
    {
        ...
        public ResponseMessageResult GetOrderList(string keyword, int status, int page, int pagesize, string begindate = "", string enddate = "")
        {
            ...
            if (jwtmodel.isadmin)
            {
                ...
                var query = db.Query("exec PROC_OrderList @0,@1,@2,@3,@4,@5,@6,@7", begindate, enddate, 0, jwtmodel.userid, keyword, status, page, pagesize).ToList();
