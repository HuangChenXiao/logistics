...
{
    public class CheLiangInfoController : ApiController
    {
        private EBMSystemEntities db = new EBMSystemEntities();
        JsonModel model = new JsonModel();

        public ResponseMessageResult Get(string keyword = null, string cXZDWBianMa=null)
        {
            ...
            if (!string.IsNullOrEmpty(openid))
            {
                var temp = from a in db.CheLiangInfo
                           ...
                           where (a.cXZDWBianMa == cXZDWBianMa || string.IsNullOrEmpty(cXZDWBianMa))
                           select new
                           {
                               a.cChePaiHao,
                               a.cPinPai,
