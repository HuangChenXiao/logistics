...
namespace WebAPI.Controllers.Agent
{
    public class ByAgentController : ApiController
    {
        ...
        public ResponseMessageResult GetOrderList(string keyword, int page, int pagesize, int status, string begindate = "", string enddate = "")
        {
            JwtModel jwtmodel = JwtHelper.getToken(HttpContext.Current.Request.Headers.GetValues("Authorization").First().ToString());
            if (jwtmodel.isadmin)
            {
                var db = ContextDB.Context();
                var query = db.Query("exec PROC_OrderList @0,@1,@2,@3,@4,@5,@6,@7", begindate, enddate, jwtmodel.userid,0,keyword, status, page, pagesize).ToList();
