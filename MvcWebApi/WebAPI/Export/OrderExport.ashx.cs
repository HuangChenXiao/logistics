...
namespace WebAPI.Export
{
    ...
    public class OrderExport : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var query = db.Query("exec PROC_OrderList @0,@1,@2,@3,@4,@5,@6,@7", begindate, enddate, agid, merchantid, keyword, status, page, pagesize).ToList();
            ...
            if (query.Count > 0)
            {
                ...
                table.Columns.Add("集成商户", typeof(string));
                ...
                foreach (var item in query)
                {
                    ...
                    table.Rows.Add(item.orderno, item.amount, item.servicecharge, item.realamount, item_status,
                                    item.agname, item.merchantname, item.shortname, item_paytype, item.remark, item.addtime);
                }
            }
            else
            {
